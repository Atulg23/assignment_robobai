...
let AdUnit = require('../models/Products');
...
adUnitRoutes.route('/add').post(function (req, res) {
  ...
  adUnit.save()
    .then(game => {
    res.status(200).json({'adUnit': 'Products added successfully'});
    })
    .catch(err => {
...
adUnitRoutes.route('/update/:id').post(function (req, res) {
    ...
    if (!adUnit)
      return next(new Error('Could not load Document'));
        ...
        adUnit.prod_id = req.body.prod_id;
        adUnit.quantity = req.body.quantity;
        adUnit.prod_name = req.body.prod_name;
        adUnit.cost_price = req.body.cost_price;
        adUnit.selling_price = req.body.selling_price;
